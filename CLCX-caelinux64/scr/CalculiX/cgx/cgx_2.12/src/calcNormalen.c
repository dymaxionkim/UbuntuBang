...
void calcFaceNorm_quad4(int i, int n1, int n2, int n3, int n4, int f, Nodes *node, Faces *face)
{
  ...
  static double v1[3], v2[3], v3[3];
      ...
      for(n=0; n<3; n++) { node[n1].nv[n]+=v3[n]; node[n2].nv[n]+=v3[n]; node[n3].nv[n]+=v3[n]; node[n4].nv[n]+=v3[n]; }
}
...
void calcFaceNorm_tri3(int i, int n1, int n2, int n3, int f, Nodes *node, Faces *face )
{
  ...
  static double v1[3], v2[3], v3[3];
      ...
      v_prod( v1, v2, v3 );
      v_norm( v3, face[i].side[f] );
      for(n=0; n<3; n++) { node[n1].nv[n]+=v3[n]; node[n2].nv[n]+=v3[n]; node[n3].nv[n]+=v3[n]; }
}
...
void getFaceNormalen( Faces *face, Nodes *node, Summen *anz )
{
  ...
  for (i=0; i<anz->n; i++ ) v_norm( node[node[i].nr].nv, node[node[i].nr].nv );
}
