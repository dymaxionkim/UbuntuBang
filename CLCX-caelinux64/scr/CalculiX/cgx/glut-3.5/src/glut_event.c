  ...
  if (workMask & (GLUT_EVENT_MASK_WORK | GLUT_DEVICE_MASK_WORK |
	...
	//             window->desiredWidth, window->desiredHeight);
      ...
      if (window->desiredConfMask & CWStackMode) {
        stack_mode = window->desiredStack;
	...
	if(changes.left < 0) {
	  changes.right = changes.right - changes.left;
	  changes.left = 0;
	}
	if(changes.top < 0)  {
	  changes.bottom = changes.bottom - changes.top;
	  changes.top = 0;
	}
#endif
      }
      
      // do the repositioning and moving
      SetWindowPos(window->win, 0, changes.left, changes.top, 
		   changes.right - changes.left, changes.bottom - changes.top,
		   SWP_NOZORDER);
      
      // do the Z ordering.
      SetWindowPos(window->win, stack_mode == Above ?HWND_TOP : HWND_NOTOPMOST,
		   0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOREPOSITION);
